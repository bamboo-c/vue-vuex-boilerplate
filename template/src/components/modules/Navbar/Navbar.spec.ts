...
describe('Navbar component', () => {
  ...
  before(() => {
    ...
    let appComponent = { template: '<div class="app">Home</div>' };
    let counterComponent = { template: '<div class="counter">Counter</div>' };
  ...
  describe('When clicking the counter link', () => {
    beforeEach(async () => {
      directiveTest.createComponent({ router: router });

      await directiveTest.execute((vm) => {
        let anchor = <HTMLAnchorElement>vm.$el.querySelector('ul.nav li a[href="#/counter"]');
        anchor.click();
      });
    });

    it('should render correct counter contents', async () => {
      await directiveTest.execute((vm) => {
        expect(vm.$el.querySelector('div.counter').textContent).to.equal('Counter');
      });
    });
  });
  ...
  describe('When clicking the list link', () => {
    ...
    it('should render correct list contents', async () => {
      await directiveTest.execute((vm) => {
